    ...
    return (
        <div className={tripBuilder.container}>
            <div className={tripBuilder.viewSelect}>
                <button onClick={() => dispatch(setMapView())} className={!useList ? tripBuilder.active : ""}>Map</button>
                |
                <button onClick={() => dispatch(setListView())} className={useList ? tripBuilder.active : ""}>List</button>
            </div>
            {useList ? <List /> : <Map/>}
        </div>
    )
